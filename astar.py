...
class Graph:
    ...
    """
    def getChildren(self, position, board):
        tempPiece = copy.deepcopy(position)
        for i in range(len(PIECES[tempPiece['shape']])):
            if tetris.isValidPosition(board, tempPiece, adjX=1):
            if tetris.isValidPosition(board, tempPiece, adjX=-1):
            if tetris.isValidPosition(board, tempPiece, adjY=1):
            if tetris.isValidPosition(board, tempPiece, adjY=-1):
...
def astar(graph, start, goal, board):
    ...
    while openList:
        ...
        if position == goal:
            break
        for childPosition, actionCost in graph.getChildren(position, board):
